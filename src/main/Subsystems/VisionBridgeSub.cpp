...
void VisionBridgeSub::Listen() {
	...
	unsigned int sockAddrSize; // size of socket address structure
	int sockListener;       // socket file descriptor
	char buf[MAX_PACKET_SIZE + 1];

	// Set up the local address.
	sockAddrSize = sizeof(sockaddr_in);
	//memset(static_cast<void*>(&serverAddr),0, sockAddrSize);
...
void VisionBridgeSub::SetBoiler(double x1, double y1, double x2, double y2) {
	...
	frc::SmartDashboard::PutNumber("BoilerY1", y1);
	frc::SmartDashboard::PutNumber("BoilerX2", x2);
	frc::SmartDashboard::PutNumber("BoilerY2", y2);
